...
def test_all_defined_aliases():
    ...
    for alias, expected_id in aliases_to_test.items():
        ...
        assert config.id == expected_id, (
            f"Alias '{alias}' should resolve to '{expected_id}'"
        )
...
def test_invalid_alias_structure_logged(monkeypatch, tmp_path, caplog):
    ...
    # Missing model_id
    aliases_data = {
        "model_aliases": {"bad_alias": {"parameters": {"temperature": 0.5}}}
    }
    aliases_file.write_text(json.dumps(aliases_data))
...
def test_alias_pointing_to_unknown_model(monkeypatch, tmp_path):
    ...
    aliases_data = {
        "model_aliases": {"broken": {"model_id": "nonexistent-model", "parameters": {}}}
    }
    aliases_file.write_text(json.dumps(aliases_data))
...
def test_validate_model_id_with_alias(monkeypatch, tmp_path):
    ...
    aliases_data = {
        "model_aliases": {
            ...
            "invalid_alias": {"model_id": "nonexistent", "parameters": {}},
        }
    }
    aliases_file.write_text(json.dumps(aliases_data))
...
def test_default_aliases_file_exists():
    ...
    default_file = (
        project_root / "adh_cli" / "config" / "defaults" / "model_aliases.json"
    )

    # File must exist
    assert default_file.exists(), (
        "Default model_aliases.json file is missing from package"
    )
...
def test_invalid_default_alias_logged(monkeypatch, tmp_path, caplog):
    ...
    aliases_data = {
        "model_aliases": {
            "valid": {"model_id": ModelRegistry.FLASH_LATEST.id, "parameters": {}},
            "invalid_no_model": {
